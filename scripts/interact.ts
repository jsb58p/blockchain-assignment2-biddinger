...
const PRIVATE_KEY_RAW = (process.env.PRIVKEY ||
"").replace(/^0x/, "");
...
async function main() {
if (!RPC_URL || !CHAIN_ID || !PRIVATE_KEY_RAW) throw new
Error("Missing envs");
if (!TOKEN) throw new
Error("Set TOKEN in scripts/interact.ts");
...
const acct2 = getAddress(ACCT2 || account.address);
const balances = async (label: string) => {
const b1 = await publicClient.readContract({ address:
getAddress(TOKEN), abi, functionName: "balanceOf", args:
[account.address] });
const b2 = await publicClient.readContract({ address:
getAddress(TOKEN), abi, functionName: "balanceOf", args: [acct2]
});
console.log(`${label} | Deployer: ${formatUnits(b1 as
bigint, DECIMALS)} CAMP | Acct2: ${formatUnits(b2 as bigint,
DECIMALS)} CAMP`);
};
...
console.log("HASHES:", JSON.stringify({ tx1, tx2, tx3 }, null,
2));
}
